...
kind: Service
metadata:
  name: te-agent-teastore-multicloud-gke
  namespace: thousandeyes
spec:
  type: ClusterIP
  selector:
    app: te-agent-teastore-multicloud-gke
  ports:
...
spec:
  selector:
    matchLabels:
      app: te-agent-teastore-multicloud-gke
  serviceName: te-agent-teastore-multicloud-gke
  replicas: 1
  template:
    metadata:
      labels:
    ...
    spec:
      ...
      nodeSelector:
        tehost: host2
      containers:
      - name: te-agent-teastore-multicloud
        ...
        env:
          ...
          - name: AGENT_NAME
            valueFrom:
              fieldRef:
                apiVersion: v1
                # StatefulSet metadata.name will be te-agent-teastore-multicloud-gke-0
